...
#特效，PortalCooldown归零（超时/撞墙/近炸）就放烟花
execute as @e[type=snowball,tag=missile,nbt={PortalCooldown:0}] at @s run summon minecraft:firework_rocket ~ ~ ~ {FireworksItem:{id:"firework_rocket",components:{fireworks:{flight_duration:6,explosions:[{shape:"small_ball"}]}}},LifeTime:0}
#提取雪球发射者信息,然后执行伤害函数，然后删除雪球防止BUG
