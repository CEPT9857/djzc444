...
execute if score boost 3 matches 0 run tellraw @a [{translate:"djzc.msg.load_unboost",fallback:"检测到您未处于boost模式，点击可启用boost",color:"red",hover_event:{action:"show_text",value:{translate:"djzc.msg.load_unboost.desc",fallback:"启用后，将关闭怪物生成等游戏规则，这是我们推荐的游戏模式"}},click_event:{action:"run_command",command:"function djzc444:option/boost"}}]
execute if score boost 3 matches 1 run tellraw @a [{translate:"djzc.msg.load_boost",fallback:"检测到您已处于boost模式，点击可禁用boost",color:"red",hover_event:{action:"show_text",value:{translate:"djzc.msg.load_boost.desc",fallback:"禁用后，游戏将启用怪物破坏地形等规则，建议您在此之前备份存档"}},click_event:{action:"run_command",command:"function djzc444:option/unboost"}}]
#加载信息
