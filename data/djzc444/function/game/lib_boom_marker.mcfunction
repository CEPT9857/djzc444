...
#在可召唤轰炸的区域内，可以召唤轰炸标记（debug模式中，因为同时运行两条指令，轰炸塔区域会重复召唤轰炸标记）
execute as @a[x=-206,y=84,z=-723,dx=5,dy=5,dz=5,scores={r_click5=1..}] at @s run summon minecraft:marker ~ ~1.5 ~ {Tags:["mark1"]}
execute as @a[x=-378,y=97,z=-887,dx=3,dy=5,dz=3,scores={r_click5=1..}] at @s run summon minecraft:marker ~ ~1.5 ~ {Tags:["mark1"]}
execute if score debug 3 matches 1 as @a[scores={r_click5=1..}] at @s run summon minecraft:marker ~ ~1.5 ~ {Tags:["mark1"]}
#data modify，获取玩家的朝向
execute as @e[type=minecraft:marker,tag=mark1] run data modify entity @s Rotation set from entity @p[scores={r_click5=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick"}}] Rotation
#扣掉轰炸指示器的耐久（debug模式不扣）
execute as @e[tag=mark1] at @s unless score debug 3 matches 1 run item modify entity @p[distance=..1.5,nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick"}}] weapon djzc444:damage_half
#重置分数
execute as @a[scores={r_click5=1..}] run scoreboard players set @s r_click5 0
#设置分数，挂接到常驻时钟
execute as @e[tag=mark1] run scoreboard players set @s time2 20
#修改标签，表明已经获取过朝向
