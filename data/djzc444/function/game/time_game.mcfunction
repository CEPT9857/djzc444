...
execute if score 秒 time <= 秒 time2 run scoreboard players set 秒 time 2060
#如果秒<=2000，秒数重置为60，时间减一分钟

execute if score 分 time < 分 time2 run function djzc444:game/end_ct
execute if score 分 time < 分 time2 run scoreboard players set 分 time 3999
#如果分钟小于3000，分钟重置为3999，并判定T战败