...
#把时间塞进命令存储，在对话框里可以调用
execute store result storage time minute int 1 run scoreboard players get 分 djzc.time
execute store result storage time second int 1 run scoreboard players get 秒 djzc.time
#时间的存储
execute store result storage type_manage minute int 1 run scoreboard players get 分 djzc.time
execute store result storage type_manage second int 1 run scoreboard players get 秒 djzc.time
#兵种管理系统的存储
